...
class TestTrainingDataGeneration:
    def test_shape_and_ranges(self) -> None:
        ...
        X, y = generate_training_data(50, cfg, geom, mat, UK_TYPICAL_INSTALLATION,
                                       seed=0)
        assert X.shape == (50, 3)
        assert y.shape == (50, 1)
        # Inputs lie in the configured ranges
        assert (X[:, 0] >= cfg.I_range_A[0]).all() and (X[:, 0] <= cfg.I_range_A[1]).all()
        assert (X[:, 1] >= cfg.ambient_range_C[0]).all() and (X[:, 1] <= cfg.ambient_range_C[1]).all()
        assert (X[:, 2] >= cfg.rho_t_range_KmW[0]).all() and (X[:, 2] <= cfg.rho_t_range_KmW[1]).all()
...
class TestTrainingLoopBasics:
    ...
    def test_loop_runs_and_returns_result(self) -> None:
        cfg = TrainingConfig(n_train=200, n_val=50, n_epochs=20,
                              batch_size=64, weight_update_every=10)
        ...
        assert all(
            __import__("math").isfinite(v) for v in result.history["loss_data"]
        )
...
class TestGF003Target:
    ...
    def test_meets_rmse_target(self) -> None:
        cfg = TrainingConfig(n_train=2000, n_val=400, n_epochs=600,
                              batch_size=128)
        model = IECSurrogatePINN(n_hidden=4, hidden_size=64, n_freqs=4)
        result = train_pinn(model, cfg, verbose=False)
        assert result.best_val_rmse_C <= 0.5, (
